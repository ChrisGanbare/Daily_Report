...
# 尝试导入数据库模块
try:
    from src.core.db_handler import USE_PYMYSQL, DatabaseHandler
...
class TestCoreDbHandler(BaseTestCase):
    ...
    def test_databasehandler_disconnect_with_mysql_connector_connection(
        self, mock_mysql
    ):
        """
    ...
    def test_databasehandler_get_device_and_customer_info(self):
        ...
        result = self.db_handler.get_device_and_customer_info(
            "DEV001", "SELECT id, customer_id FROM device WHERE code = %s"
        )
        self.assertEqual(result, (1, 100))
        mock_cursor.execute.assert_called_once_with(
            "SELECT id, customer_id FROM device WHERE code = %s", ("DEV001",)
        )
    ...
    @patch("src.core.db_handler.mysql.connector")
    def test_databasehandler_get_customer_name_by_device_code_not_found(
        self, mock_mysql
    ):
        """
